...
<aside class="z-20 flex-shrink-0 hidden w-64 overflow-y-auto bg-white md:block" aria-label="aside">
    <div class="text-serv-bg">
        ...
        <ul>
            ...
            {{-- Logout --}}
            <li class="relative px-6 py-3">
                <a class="inline-flex items-center w-full text-sm font-light transition-colors duration-150 hover:text-gray-800" href="{{ route('logout') }}" 
                ...
                <form action="{{ route('logout') }}" id="logout-form" class="hidden" method="POST">
                    @csrf
                </form>
            </li>
        </ul>
