                        ...
                        <tbody class="bg-white">
                            <tr class="mb-10 text-gray-700">
                                <td class="px-1 py-5 text-sm w-2/8">
                                    <div class="flex items-center text-sm">
                                        <div class="relative w-10 h-10 mr-3 rounded-full md:block">
                                            @if ($order->user_freelancer->user_detail->photo != NULL)
                                                <img class="object-cover w-full h-full rounded-full"
                                                src="{{ Storage::url($order->user_freelancer->user_detail->photo) }}" alt=""
                                                loading="lazy" />
                                            @else
                                                <span class="inline-block w-full h-full overflow-hidden bg-gray-100 rounded-full">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-gray-400">{{ $order->user_freelancer->user_detail->contact_number ?? '' }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="w-2/6 px-1 py-5">
                                    <div class="flex items-center text-sm">
                                        <div class="relative w-10 h-10 mr-3 rounded-full md:block">
                                            @if ($order->service->service_thumbnail->first()->thumbnail != NULL)
                                                <img class="object-cover w-full h-full rounded"
                                                src="{{ Storage::url($order->service->service_thumbnail->first()->thumbnail) }}" alt="thumbnail"
                                                loading="lazy" />    
                                            @else
                                                <img class="object-cover w-full h-full rounded"
                                                src="{{ asset('assets/images/empty-illustration.svg') }}" alt="thumbnail"
                                                loading="lazy" />    
                                            @endif
                                            <div class="absolute inset-0 rounded-full shadow-inner" aria-hidden="true"></div>
                                        </div>
                    ...
                    <div class="flex p-8 border border-gray-200 rounded-lg bg-serv-upload-bg h-128">
                        <div class="m-auto text-center">
                            ...
                            <div class="relative mt-0 md:mt-6">
                                @if (isset($order->file))
                                    <a href="{{ Storage::url($order->file) }}" class="px-4 py-2 mt-2 text-left text-gray-700 rounded-xl bg-serv-hr">
                                        Download File
                                    </a>
                                @else
                    ...
                    <form action="#" method="POST">
                        <div class="">
                            <div class="p-1 mt-5">
                                <div class="grid grid-cols-6 gap-6">
                                    <div class="col-span-6">
                                        ...
                                        <textarea type="text"
                                            placeholder="{{ empty($order->note) ? 'Tidak ada catatan dari freelancer' : '' }}"
                                            name="service-name" id="service-name" disabled autocomplete="service-name"
                            ...
                            <div class="px-1 py-4 text-right">
                                <a type="button" href="{{ route('member.request.index') }}"
                                    class="inline-flex justify-center px-4 py-2 mr-4 text-sm font-medium text-gray-700 bg-white border border-gray-600 rounded-lg shadow-sm hover:bg-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300">
                                    Back
                                </a>

                                @if ($order->order_status_id == 2 && isset($order->file))
                                    <a type="button" href="{{ route('member.request.approve', $order->id) }}"
                                        class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-lg shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500"
                                        onclick="return confirm('Are you sure want to approve this request?')">
                                        Approve
                                    </a>
                                @endif                                
                            </div>
