            ...
            <div class="p-3 my-4 bg-gray-100 rounded-lg image-gallery" x-data="gallery()">
                ...
                <div class="flex overflow-x-scroll hide-scroll-bar dragscroll">
                    <div class="flex mt-2 flex-nowrap">
                        @php $active = 0 @endphp
                        @forelse ($service_thumbnails as $thumbnail)
                            @php $active++ @endphp
                            <img :class="{'border-4 border-serv-button': active === {{ $active }}}"
                            @click="changeThumbnail('{{ Storage::url($thumbnail->thumbnail) }}', 1)"
                            src="{{ Storage::url($thumbnail->thumbnail) }}" alt="thumbnail"
                            class="inline-block mr-2 rounded-lg cursor-pointer w-36 object-cover">
                        @empty
                            {{-- Empty State --}}
                        @endforelse
                    </div>
                ...
                <div x-data="{ tab: window.location.hash ? window.location.hash.substring(1) : 'description' }"
                    ...
                    <div x-show.transition.duration.500ms="tab === 'description'" class="leading-8 text-md">
                        ...
                        <ul class="mb-4 list-check">
                            @forelse ($service_advantages as $service_advantage)
                                <li class="pl-10 my-2">{{ $service_advantage->advantage }}</li>
                            @empty
                                {{-- Empty State --}}
                            @endforelse
                        </ul>
                    ...
                    <div x-show.transition.duration.500ms="tab === 'seller'" class="leading-8 text-md">
                            ...
                            <div class="flex items-center col-span-12 p-2 lg:col-span-6">
                                <div class="flex items-center space-x-4">
                                    @if ($service->user->user_detail->photo != NULL)
                                        <img src="{{ Storage::url($service->user->user_detail->photo) }}"
                        ...
                        <ul class="mb-8 list-check">
                            @forelse ($service->user->user_detail->user_experience as $user_experience)
                                <li class="pl-10 my-2">{{ $user_experience->experience ?? '' }}</li>
                            @empty
                                {{-- Empty State --}}
                            @endforelse                            
                        </ul>
            ...
            <div class="mb-4 border rounded-lg border-serv-testimonial-border">
                ...
                <div class="px-4 pt-4 pb-2 features-list">
                    <ul class="mb-4 text-sm list-check">
                        @forelse ($user_advantages as $user_advantage)
                            <li class="pl-10 my-4">{{ $user_advantage->advantage }}</li>
                        @empty
                            {{-- Empty State --}}
                        @endforelse
                    </ul>
                ...
                <div class="px-4 pb-4 booking">
                    @auth
                        <a href="{{ route('landing.booking', $service->id) }}"
                    ...
                    @guest
                        <a onclick="toggleModal('loginModal')"
                            class="block px-12 py-4 my-2 text-lg font-semibold text-center text-white bg-serv-button rounded-xl cursor-pointer">
                            Log In
                        </a>
                    @endguest       
                </div>
...
@push('after-script')
    <script>
        function gallery() {
            return {
                featured: '{{ Storage::url($service_thumbnails->first()->thumbnail) }}',
                active: 1,
